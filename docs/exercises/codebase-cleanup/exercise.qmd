...
```sh
conda create -n rps-env python=3.10
conda activate rps-env
```

Ideally add these environment setup instructions to the README file.

## Starter Code


Create a file in the "my_project" directory called "game.py", and place inside the following contents (and save the file):

```python
# the "my_module/game.py" file ...
...
We received some reports the winner determination logic is inaccurate. It may be hard to see where the bug is just by looking at the code. We will probably want to test the code, but before we do we will need to refactor it into a custom function that can be tested in isolation.

**Challenge 4**: Refactor the winner determination logic into a stand-alone function called `determine_outcome`. The function should accept the user choice and the computer choice as input parameters (perhaps called `u` and `c`), and should return a string denoting the outcome (i.e. "USER WINS" or "COMPUTER WINS", or "TIE GAME").
...
**Challenge 5**: Implement an automated test to ensure the winner determination logic produces accurate outcomes, given all possible combinations of the inputs.
...
Notice the tests may be failing for now, until we fix the bug later.

### Improving Quality / Fixing Bugs

Let's actually improve the functionality and fix the bug.

**Challenge 6**: Notice the existing winner determination logic produces some inaccurate outcomes, and update the logic to produce accurate outcomes.
...
**Challenge 7**: Document the new winner determination function, using a docstring. Include a description of the function's responsibilities, the parameters it accepts, and the values it returns. Consider formatting your docstring using google-style or numpy-style docstring syntax.
...
**Challenge 8**: Refactor the complex logic to use less computational steps. HINT: you can use a nested if statement, or better yet a nested dictionary to model the winners.
...
10. Create a new file called "gui_game.py" or "web_game.py", to implement a GUI version of the game, or a web-based version of the game, respectively. For a GUI you can use the `tkinter` package or the `pysimplegui` package. For a web based game, you can use the `flask` package.
