...
SPOLER ALERT / HINT: here is a valid implementation of some Python code to accomplish the objectives.

Create a file in the "app" directory called "game_loop.py" and place the following contents inside:

```python
# this is the "app/game_loop.py" file...
...
while True:
    ...
    if outcome == "WIN":
        win_count += 1
    print("CURRENT STATS... WINS:", win_count, "GAMES:", game_count, "WIN PCT:", round(win_count/game_count, 2))
...
Let's use a test-driven development approach, whereby we will write the tests first, and then write the application code to make the tests pass.
...
Run the tests to see they fail, then write some code in the "app/game_loop.py" file to make the tests pass. The "app/game_loop.py" file should also run on its own, asking for user inputs to play the game, and asking them whether they want to play again. After they say they are done, it should print the final game count and win statistics.
...
class GameLoop:
    pass # TODO: write the code to make the tests pass
...
if __name__ == "__main__":
    ...
    #game.play() # to play a single game

    game = GameLoop()
    print(game.welcome_message)
    game.play_until_satisfied() # to play as many games as the user wants


```


Spoiler: see [solutions](./solutions.py).
